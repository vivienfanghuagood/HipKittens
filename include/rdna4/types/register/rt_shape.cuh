 ...
 * pick per tile. gfx12 has exactly one matrix instruction geometry,
 ...
 * the reduction axis. gfx11 put the whole K=16 vector in every operand lane, so
 * a run there was the full 16; gfx12 splits K between the wave halves, so a run
 * is 8. The member is vestigial -- nothing in this tree or the tests reads it --
 * and rt_16x16 keeps the upstream 16 so the duck's spelling matches CDNA's.
 */
namespace rt_shape {
...
/// gfx12's base tile is square, so transposing one does not change its shape.
