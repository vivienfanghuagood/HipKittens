...
__device__ inline void load(ST& dst, const GL& src, const COORD& idx)
{
    ...
    // 4 VGPRs, so a batch of 8 costs 32 VGPRs out of the 256 a gfx12 wave gets.
    ...
    // Inherited from the rdna3 tree and not retuned: gfx12's halved operand
    // fragments free up VGPRs that a larger batch could spend, but picking the
    // number without a machine to measure on would be guessing.
    ...
    for (int i = 0; i < big_calls; i++) {
        ...
        // The rdna3 tree has a BUILTINS_ONLY escape hatch here that calls
        // __builtin_amdgcn_s_waitcnt(0). That builtin encodes the *combined*
        // gfx11 S_WAITCNT, which gfx12 replaced with one instruction per
        // counter, so there is no builtin spelling of this wait on gfx12 and
        // the alternative is dropped rather than left to rot.
        vmem_load_wait<0>();
        ...
        for(int j = 0; j < small_calls; j++) {
            ...
            if (row < dst.rows) {
                // One ds_store_b128: `col` is a whole number of float4s, so the
...
__device__ inline void load_global_to_register_buffer(float4* reg_buffer, const int buffer_size, const GL& src, const COORD& idx, const ST& dst_template) {
    ...
    // The config word is the gfx12 one -- see make_srsrc; the gfx9 word the CDNA
