...
namespace kittens {
    ...
    struct KITTENS_DEFAULT_ALIGN st {
        ...
        //
        // Three properties make this the right constant on gfx12, and all three
        // differ from what the CDNA trees do:
        //
        //  * 16-byte granule, not 8. The whole point is that a lane may read its
        //    operand row with one ds_load_b128; that is only correct if 16
        ...
        //    would not. On gfx12 all 32 lanes want distinct 16-byte granules
        //    (the wave halves split K rather than mirroring, so there is no
        //    broadcast to collapse), and the permutation spreads them over the
        //    eight bank groups four lanes each -- the 4-cycle minimum for the
        //    512 bytes moved. The derivation is written out in
        //    ops/warp/memory/tile/shared_to_register.cuh. !! Hand-derived, not
        //    measured: no gfx12 part has run this. !! On gfx11 the same
        //    constant hit that arch's 2-cycle minimum, which was measured.
